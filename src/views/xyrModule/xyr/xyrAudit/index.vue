<template>
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" highlight-current-row style="width: 100%">
      ...
      <el-table-column width="140px" align="center" label="学科建设总分">
        <template slot-scope="scope">
          <span>{{ scope.row.xyrModule.xyr ? scope.row.xyrModule.xyr.xkjs.sum : 0}}</span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="研究生工作总分" width="140px">
        <template slot-scope="scope">
          <span>{{ scope.row.xyrModule.xyr ? scope.row.xyrModule.xyr.yjsgz.sum : 0}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="人才引进总分" width="140px">
        <template slot-scope="scope">
      ...
      <el-table-column class-name="status-col" align="center" label="状态" width="80px">
        <template slot-scope="scope">
          <!-- <el-tag :type="scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.xyrModule.teaMoudelAuditRecord[0] ? scope.row.xyrModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.xyrModule.xyr.auditRecord ? (scope.row.xyrModule.xyr.auditRecord[0] ? scope.row.xyrModule.xyr.auditRecord[0].auditStatus : '待审核') : '待审核'))| statusFilter">
            {{ scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.xyrModule.teaMoudelAuditRecord[0] ? scope.row.xyrModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.xyrModule.xyr.auditRecord ? (scope.row.xyrModule.xyr.auditRecord[0] ? scope.row.xyrModule.xyr.auditRecord[0].auditStatus : '待审核') : '待审核'))}}
          </el-tag> -->
          <el-tag :type="scope.row.xyrModule.xyr.status | statusFilter">{{scope.row.xyrModule.xyr.status}}</el-tag>
        </template>
       ...
       <el-table-column width="100px" align="center" label="总分数">
        <template slot-scope="scope">
          <span>
            {{ (scope.row.xyrModule.xyr ?  scope.row.xyrModule.xyr.xkjs.sum : 0)
             + (scope.row.xyrModule.xyr ? scope.row.xyrModule.xyr.rcyj.sum : 0) 
             + (scope.row.xyrModule.xyr ? scope.row.xyrModule.xyr.yjsgz.sum : 0) 
            }}
          </span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleAudit(row) {
      console.log('row :>> ', row);
      if (row.xyrModule.xyr.status == '待审核') {
        this.dialogTableVisible = true;
    ...
    //审核提交接口调用
    handleSubmit(params) {
      this.form.xyrModule.xyr.auditRecord.unshift(params);
      this.form.xyrModule.xyr.status = params.auditStatus;
