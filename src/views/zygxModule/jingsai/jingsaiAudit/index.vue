<template>
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" highlight-current-row style="width: 100%">
      ...
      <el-table-column  align="center" label="参加、指导学科竞赛总分" width="250px">
        <template slot-scope="scope">
      ...
      <el-table-column class-name="status-col" align="center" label="状态" width="80px">
        <template slot-scope="scope">
          <!-- <el-tag :type="scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.zygxModule.teaMoudelAuditRecord[0] ? scope.row.zygxModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.zygxModule.jingsai.auditRecord ? (scope.row.zygxModule.jingsai.auditRecord[0] ? scope.row.zygxModule.jingsai.auditRecord[0].auditStatus : '待审核') : '待审核'))| statusFilter">
            {{ scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.zygxModule.teaMoudelAuditRecord[0] ? scope.row.zygxModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.zygxModule.jingsai.auditRecord ? (scope.row.zygxModule.jingsai.auditRecord[0] ? scope.row.zygxModule.jingsai.auditRecord[0].auditStatus : '待审核') : '待审核'))}}
          </el-tag> -->
          <el-tag :type="scope.row.zygxModule.jingsai.status | statusFilter">{{scope.row.zygxModule.jingsai.status}}</el-tag>
        </template>
      </el-table-column>    
       <el-table-column
        label="操作"
...
export default {
  ...
  methods: {
    ...
    handleAudit(row) {
      console.log('row :>> ', row);
      if (row.zygxModule.jingsai.status == '待审核') {
        this.dialogTableVisible = true;
    ...
    //审核提交接口调用
    handleSubmit(params) {
      this.form.zygxModule.jingsai.auditRecord.unshift(params);
      this.form.zygxModule.jingsai.status = params.auditStatus;
