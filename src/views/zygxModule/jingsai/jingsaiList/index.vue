<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list"  fit highlight-current-row style="width: 100%">
      ...
      <el-table-column type="expand">
        <template slot-scope="scope" > 
          <el-form label-position="left" inline class="demo-table-expand">
            <el-form-item label="参加、指导学科竞赛:">
              <div v-for="(item,key) in scope.row.zygxModule.jingsai ? scope.row.zygxModule.jingsai.canjiajingsai.item : []">
              ...
              <span class="data-items">总分: {{scope.row.zygxModule.jingsai ? scope.row.zygxModule.jingsai.canjiajingsai.sum : 0}}</span>
            </el-form-item>
            <el-form-item label="审核记录:">
              ...
              <div v-for="(item,key) in scope.row.zygxModule.jingsai ? scope.row.zygxModule.jingsai.auditRecord : []">
                <span class="data-items">审核人: {{item.auditPerson ? item.auditPerson :'暂无'}}</span>
                <span class="data-items">审核时间: {{item.auditTime ? item.auditTime : 0 | formateDate}}</span>
                <span class="data-items">状态: {{item.auditStatus ? item.auditStatus : '待审核'}}</span>
                <span class="data-items">审核理由: {{item.auditReason ? item.auditReason : '暂无'}}</span>
      ...
      <el-table-column align="center" label="参加、指导学科竞赛总分" width="235">
        <template slot-scope="scope">
          <span>{{ scope.row.zygxModule.jingsai ? scope.row.zygxModule.jingsai.canjiajingsai.sum : 0}}</span>
        </template>
      </el-table-column>
       <el-table-column align="center" label="此模块总分" width="135">
        <template slot-scope="scope">
          <span>{{ scope.row.zygxModule.jingsai ? scope.row.zygxModule.jingsai.jingsaiScoreSum: 0}}</span>
        </template>
      </el-table-column>
      <el-table-column class-name="status-col" label="状态" align="center" width="80">
        <template slot-scope="scope">
          <!-- <el-tag :type="scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.zygxModule.teaMoudelAuditRecord[0] ? scope.row.zygxModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.zygxModule.jingsai ? (scope.row.zygxModule.jingsai.auditRecord[0] ? scope.row.zygxModule.jingsai.auditRecord[0].auditStatus : '待审核') : '待审核')) | statusFilter">
            {{ scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.zygxModule.teaMoudelAuditRecord[0] ? scope.row.zygxModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.zygxModule.jingsai ? (scope.row.zygxModule.jingsai.auditRecord[0] ? scope.row.zygxModule.jingsai.auditRecord[0].auditStatus : '待审核') : '待审核'))}}
          </el-tag> -->
           <el-tag :type="scope.row.zygxModule.jingsai.status | statusFilter">
            {{scope.row.zygxModule.jingsai.status}}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column width="90px" align="center" label="审核人">
        <template slot-scope="scope">
          <span>
            {{ scope.row.finalAuditRecord[0] ? scope.row.finalAuditRecord[0].auditPerson : (scope.row.zygxModule.jingsai ? (scope.row.zygxModule.jingsai.auditRecord[0] ? scope.row.zygxModule.jingsai.auditRecord[0].auditPerson : '暂无') : '暂无') }}
          </span>
        </template>
      </el-table-column>
      <el-table-column width="140px" align="center" label="审核时间">
        <template slot-scope="scope">
          <span>
            {{ scope.row.finalAuditRecord[0] ? scope.row.finalAuditRecord[0].auditTime : (scope.row.zygxModule.jingsai ? (scope.row.zygxModule.jingsai.auditRecord[0] ? scope.row.zygxModule.jingsai.auditRecord[0].auditTime : '') : '') | formateDate }}
          </span>
        </template>
      </el-table-column>
    ...
    <el-dialog el-drag-dialog :visible.sync="dialogTableVisible" :title="dialogTitle">
      <el-form ref="formParams" :model="formParams">
         <el-tabs v-model="activeName">
          <el-tab-pane label="参加、指导学科竞赛" name="third">
             <div v-for="(item,key) in (formParams.zygxModule.jingsai ? formParams.zygxModule.jingsai.canjiajingsai.item : [])" style="position:relative">
              ...
              <div class="block" style="border-bottom:1px dashed;margin-top:20px">
                <el-form-item label="项目类型" prop="level">  
                  <el-cascader
                    v-model="item.level"
                    :options="canjiajingsai_options"
                    placeholder="选择项目类型"
                    :props="{ expandTrigger: 'hover' }"
                    @change="handlecanjiajingsaiChange">
                  </el-cascader>           
                </el-form-item>
                ...
                <el-form-item label="上传附件">
                  <input type="file" @change="fileSelect(item)" ref="canjiajingsaifile">
                  <el-button type="primary" plain size="mini" @click="submitFile(item)">上传</el-button>
                </el-form-item>
              </div>
            </div>
            <el-form-item label="总分" style="margin-bottom:-20px">         
                {{ formParams.zygxModule.jingsai ? formParams.zygxModule.jingsai.canjiajingsai.sum : 0}} 
            </el-form-item>
            <el-button type="text" @click="AddcanjiajingsaiItem" style="position:relative;left:70%;top:-30px">继续添加 </el-button>  
   ...
   formParams: {
        ...
        zygxModule: {
          jingsai: {
              ...
              jingsaiScoreSum: 0,//教学工程及其他模块总分
          },
          jingsaiStatus:'待审核',//教学教研考评模块审核状态
  ...
  methods: {
    ...
    getBaseForm() {
      ...
      if (id) {
        ...
        console.log('res :>> ', res);
        if (res.result.length != 0 && res.result[0].zygxModule.jingsai)  {
    ...
    handleUpdate(row) {
      console.log('row :', row);
      if (row.zygxModule.jingsai.status == '待审核' || row.zygxModule.jingsai.status == '驳回') {
      this.formParams = row;
      this.dialogTableVisible = true;
    ...
    handleDelete(row) {
      console.log('row :', row);
      if (row.zygxModule.jingsai.status == '待审核' || row.zygxModule.jingsai.status == '已完成') {
    ...
    handlecanjiajingsaiChange(value) {
      console.log('value :>> ', value);
      this.formParams.zygxModule.jingsai.canjiajingsai.sum = 0;
      for (let i of this.formParams.zygxModule.jingsai.canjiajingsai.item) {
        this.formParams.zygxModule.jingsai.canjiajingsai.sum += i.level ? i.level[1] : 0;
      }
    },
    ...
    AddcanjiajingsaiItem() {
       this.formParams.zygxModule.jingsai.canjiajingsai.item.push({
            sign:'canjiajingsai',
            level:'',//项目名称
            content:'',
            date:'',//批准日期
    ...
    deleteItem(value) {
      ...
      switch (value.sign) {
        case 'canjiajingsai': 
          target = this.formParams.zygxModule.jingsai.canjiajingsai;
    ...
    fileSelect(item) {
      ...
      switch (item.sign) {
        case 'canjiajingsai':
          fileSign = this.$refs.canjiajingsaifile;
      }
      let file = [];
    ...
    handleSubmit(formName) {
      const id = this.$store.state.user._id;
      this.formParams.zygxModule.jingsai.jingsaiScoreSum = 
      this.formParams.zygxModule.jingsai.canjiajingsai.sum  ;
      ...
      if (!id) {
        ...
        this.form.zygxModule.jingsai = this.formParams.zygxModule.jingsai;
    ...
    UpdateSubmit(formName) {
     ...
     this.formParams.zygxModule.jingsai.status = '待审核';
