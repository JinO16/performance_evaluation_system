<template>
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" highlight-current-row style="width: 100%">
      ...
      <el-table-column width="150px" align="center" label="专业（平台）建设总分">
        <template slot-scope="scope">
          <span>{{ scope.row.zygxModule.zyjs ? scope.row.zygxModule.zyjs.zhuanye.sum : 0}}</span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="学术交流总分" width="150px">
        <template slot-scope="scope">
      ...
      <el-table-column class-name="status-col" align="center" label="状态" width="80px">
        <template slot-scope="scope">
          <!-- <el-tag :type="scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.zygxModule.teaMoudelAuditRecord[0] ? scope.row.zygxModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.zygxModule.zyjs.auditRecord ? (scope.row.zygxModule.zyjs.auditRecord[0] ? scope.row.zygxModule.zyjs.auditRecord[0].auditStatus : '待审核') : '待审核'))| statusFilter">
            {{ scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.zygxModule.teaMoudelAuditRecord[0] ? scope.row.zygxModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.zygxModule.zyjs.auditRecord ? (scope.row.zygxModule.zyjs.auditRecord[0] ? scope.row.zygxModule.zyjs.auditRecord[0].auditStatus : '待审核') : '待审核'))}}
          </el-tag> -->
          <el-tag :type="scope.row.zygxModule.zyjs.status | statusFilter">{{scope.row.zygxModule.zyjs.status}}</el-tag>
        </template>
      </el-table-column>    
       <el-table-column
        label="操作"
...
export default {
  ...
  methods: {
    ...
    handleAudit(row) {
      console.log('row :>> ', row);
      if (row.zygxModule.zyjs.status == '待审核') {
        this.dialogTableVisible = true;
    ...
    //审核提交接口调用
    handleSubmit(params) {
      this.form.zygxModule.zyjs.auditRecord.unshift(params);
      this.form.zygxModule.zyjs.status = params.auditStatus;
