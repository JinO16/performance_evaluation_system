  ...
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" highlight-current-row style="width: 100%">
      ...
      <el-table-column class-name="status-col" align="center" label="状态" width="80px">
        <template slot-scope="scope">
          <!-- <el-tag :type="scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.xsgzModule.teaMoudelAuditRecord[0] ? scope.row.xsgzModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.xsgzModule.huojiang.auditRecord ? (scope.row.xsgzModule.huojiang.auditRecord[0] ? scope.row.xsgzModule.huojiang.auditRecord[0].auditStatus : '待审核') : '待审核'))| statusFilter">
            {{ scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.xsgzModule.teaMoudelAuditRecord[0] ? scope.row.xsgzModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.xsgzModule.huojiang.auditRecord ? (scope.row.xsgzModule.huojiang.auditRecord[0] ? scope.row.xsgzModule.huojiang.auditRecord[0].auditStatus : '待审核') : '待审核'))}}
          </el-tag> -->
          <el-tag :type="scope.row.xsgzModule.huojiang.status | statusFilter">{{scope.row.xsgzModule.huojiang.status}}</el-tag>
        </template>
      </el-table-column>    
       <el-table-column
        label="操作"
...
export default {
  ...
  methods: {
    ...
    handleAudit(row) {
      console.log('row :>> ', row);
      if (row.xsgzModule.huojiang.status == '待审核') {
        this.dialogTableVisible = true;
    ...
    //审核提交接口调用
    handleSubmit(params) {
      this.form.xsgzModule.huojiang.auditRecord.unshift(params);
      this.form.xsgzModule.huojiang.status = params.auditStatus;
