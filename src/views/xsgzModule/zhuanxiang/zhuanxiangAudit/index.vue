<template>
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" highlight-current-row style="width: 100%">
      ...
      <el-table-column width="300px" align="center" label="部门">
        <template slot-scope="scope">
          <span>{{ scope.row.department }}</span>
        </template>
      </el-table-column>
      <el-table-column width="150px" align="center" label="专项工作总分">
        <template slot-scope="scope">
          <span>{{ scope.row.xsgzModule.zhuanxiang ? scope.row.xsgzModule.zhuanxiang.gongzuo.sum : 0}}</span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="工作创新总分" width="150px">
        <template slot-scope="scope">
      ...
      <el-table-column class-name="status-col" align="center" label="状态" width="80px">
        <template slot-scope="scope">
          <!-- <el-tag :type="scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.xsgzModule.teaMoudelAuditRecord[0] ? scope.row.xsgzModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.xsgzModule.zhuanxiang.auditRecord ? (scope.row.xsgzModule.zhuanxiang.auditRecord[0] ? scope.row.xsgzModule.zhuanxiang.auditRecord[0].auditStatus : '待审核') : '待审核'))| statusFilter">
            {{ scope.row.finalAuditRecord[0]? scope.row.finalAuditRecord[0].auditStatus: (scope.row.xsgzModule.teaMoudelAuditRecord[0] ? scope.row.xsgzModule.teaMoudelAuditRecord[0].auditStatus :(scope.row.xsgzModule.zhuanxiang.auditRecord ? (scope.row.xsgzModule.zhuanxiang.auditRecord[0] ? scope.row.xsgzModule.zhuanxiang.auditRecord[0].auditStatus : '待审核') : '待审核'))}}
          </el-tag> -->
          <el-tag :type="scope.row.xsgzModule.zhuanxiang.status | statusFilter">{{scope.row.xsgzModule.zhuanxiang.status}}</el-tag>
        </template>
      </el-table-column>    
       <el-table-column
        label="操作"
...
export default {
  ...
  methods: {
    ...
    handleAudit(row) {
      console.log('row :>> ', row);
      if (row.xsgzModule.zhuanxiang.status == '待审核') {
        this.dialogTableVisible = true;
    ...
    //审核提交接口调用
    handleSubmit(params) {
      this.form.xsgzModule.zhuanxiang.auditRecord.unshift(params);
      this.form.xsgzModule.zhuanxiang.status = params.auditStatus;
